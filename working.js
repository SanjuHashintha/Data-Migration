...
const userIds = ['62289805b65f0eb78e26208a','6228ab05df39784c4acd881d','6228ab08df39784c4acd881f','6228ab08df39784c4acd8821']
...
const connectToMongoDB = async () => {
  ...
  try {
    ...
    for(const userId of userIds){
      const user = await collectionNfhLive.findOne({ _id: new ObjectId(userId) });
      if (!user || !user.fileUploads || user.fileUploads.length === 0) {
        // Skip to the next userId if there is no user or no fileUploads array
        console.log(`Skipping userId: ${userId} - No fileUploads found`);
      } else {
